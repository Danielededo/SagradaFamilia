...
import it.polimi.ingsw.server.utils.TimerTurn;
...
public class Server implements ServerInt {
    ...
    public void setMatch(Match match) throws RemoteException, InterruptedException {
        ...
        for (ClientInt c:listofobserver){
            ...
            if (match.getPlayers().get(client_index).isConnected()){
                ...
                try {
                    ...
                    notifyObserver("A "+match.getPlayers().get(client_index).getNickname()+" è stata assegnata casualmente la carta schema non essendo connesso");
        ...
        notifyObserver(match.getGlassWindowPlayers()+separator+"\n"+Colour.RED.escape()+"LA PARTITA HA INIZIO"+Colour.RESET);
    ...
    public void handleTurn(Round round,int z,int k,int t)throws RemoteException{
        ...
        try {
            if (round.getTurns().get(z).getOneplayer().isConnected()) {
                ...
                int menu;
                timer.schedule(timerTurn,1000*sec);
                ...
                notifyObserver("Alla fine di questo turno, "+round.getTurns().get(z).getOneplayer().toStringpublic()+separator);
            }
            else{
            ...
            match.getPlayers().get(z).setConnected(false);
            System.out.println(match.getPlayers().get(z).getNickname()+" disconnesso");
            notifyOthers(listofobserver.get(k),round.getTurns().get(z).getOneplayer().getNickname()+" salta il turno perchè è disconnesso"+separator);
            timerTurn.cancel();
            return;
        }catch (ConnectException e){
    ...
    public void vericaconnessione() throws RemoteException {
        ...
        for (ClientInt c:listofobserver){
            try {
                if((start && match.getPlayers().get(i).isConnected()) || !start)
                    c.verifyconnection();
            }catch (ConnectException e){
                if(!this.start) {
